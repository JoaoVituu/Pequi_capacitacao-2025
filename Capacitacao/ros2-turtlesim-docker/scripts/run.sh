...
docker run -it \
  ...
  $IMAGE_NAME \
  bash -c "source /opt/ros/humble/setup.bash && source /workspace/install/setup.bash && ros2 run turtlesim turtlesim_node & sleep 1 && ros2 run turtlesim turtle_teleop_key"
